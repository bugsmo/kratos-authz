...
func New(_ context.Context, opts ...OptFunc) (*State, error) {
	s := State{
		...
		projects:                  engine.Projects{},
		wildcardItem:              "*",
		authorizedProjectsMatcher: "g(r.sub,p.sub,d.dom) && (keyMatch(r.dom,p.dom) || p.dom =='*')",
	}
...
func (s *State) FilterAuthorizedPairs(_ context.Context, subjects engine.Subjects, pairs engine.Pairs) (engine.Pairs, error) {
	...
	for _, pair := range pairs {
		for _, subject := range subjects {
			if allowed, err = s.enforcer.Enforce(string(subject), string(pair.Resource), string(pair.Action), string(project)); err != nil {
				return nil, err
			} else if allowed {
